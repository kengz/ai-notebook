...
CORPUS_FILENAME = "data/europarl-v6.fr-en.en"
MAX_DOC_LENGTH = 10
...
def iter_docs(docs):
  for doc in docs:
    ...
    for part in range(n_parts):
      ...
      inp[:min(offset_begin - offset_begin, len(doc) - offset_begin)] = doc[offset_begin:offset_end]
...
byte_processor = skflow.preprocessing.ByteProcessor(max_document_length=MAX_DOC_LENGTH)
...
# Training model
estimator = skflow.TensorFlowEstimator(model_fn=get_language_model(HIDDEN_SIZE), n_classes=256, optimizer='Adam', learning_rate=0.01, steps=1000, batch_size=64, continue_training=True)
